"""
sync_history_mt5.py - One-time historical sync for enabled MT5 market-data assets.

Defaults:
- symbols: all enabled symbols in config.yaml
- range: 2017-01-01 -> today
- order: higher TF first, M1 last

Examples:
  python sync_history_mt5.py
  python sync_history_mt5.py --dry-run
  python sync_history_mt5.py --from 2017-01-01
  python sync_history_mt5.py --symbols BTCUSDc XAUUSDc XAGUSDc
  python sync_history_mt5.py --timeframes D1 H4 H1
"""
...
from symbol_config import canonical_backend_symbol, get_enabled_symbols, is_crypto_symbol
...
RESUME_STATE_FILE = os.path.join(os.path.dirname(__file__), ".sync_history_mt5_state.json")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Historical MT5 sync for all enabled market-data symbols")
    parser.add_argument(
        "--timeframes",
        nargs="+",
        default=TF_ORDER,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Backend symbol names to sync (default: all enabled symbols)",
    )
    parser.add_argument(
        "--from",
        dest="date_from",
...
def trading_days_in_range(date_from: datetime, date_to: datetime, symbol_cfg: dict) -> int:
    total = 0
    cursor = date_from
...
def estimate_bars(date_from: datetime, date_to: datetime, tf_name: str, symbol_cfg: dict) -> int:
    crypto = is_crypto_symbol(symbol_cfg)
    hours_per_day = 24 if crypto else 23
    trading_hours = trading_days_in_range(date_from, date_to, symbol_cfg) * hours_per_day
    bars_per_hour = {
        "M1": 60,
        "M5": 12,
        "M15": 4,
        ...
        "H12": 1 / 12,
        "D1": 1 / hours_per_day,
        "D3": 1 / (hours_per_day * 3),
        "W1": 1 / (hours_per_day * 7),
        "MN1": 1 / (hours_per_day * 30),
    }
...
def is_weekend_only_chunk(symbol_cfg: dict, cursor: datetime, chunk_end: datetime) -> bool:
    if is_crypto_symbol(symbol_cfg):
        return False
    return cursor.weekday() >= 5 and (chunk_end - timedelta(seconds=1)).weekday() >= 5
...
def sync_symbol_tf(
    ...
    est_bars = estimate_bars(date_from, date_to, tf_name, symbol_cfg)
    est_chunks = max(1, ((date_to - date_from).days + chunk_days - 1) // chunk_days)
    ...
    while cursor < date_to:
        ...
        if is_weekend_only_chunk(symbol_cfg, cursor, chunk_end):
            cursor = chunk_end
        ...
        if not df.empty:
            ...
            total_chunks += 1
            chunks_done = min(est_chunks, ((cursor - date_from).days // chunk_days) + 1)
...
def resolve_symbols(requested_symbols: list[str] | None) -> list[dict]:
    if not requested_symbols:
        return ALL_SYMBOLS

    requested = {canonical_backend_symbol(symbol).upper() for symbol in requested_symbols}
    symbols = [symbol_cfg for symbol_cfg in ALL_SYMBOLS if symbol_cfg["tv"].upper() in requested]

    if not symbols:
        raise SystemExit(f"[ERROR] No matching symbols: {requested_symbols}")
...
def main() -> None:
    ...
    print(f"  Symbols: {', '.join(symbol['tv'] for symbol in symbols)}")
    ...
    if args.dry_run:
        ...
        for symbol_cfg in symbols:
            for tf_name in timeframes:
                est = estimate_bars(date_from, date_to, tf_name, symbol_cfg)
                total_est += est
                print(f"  {symbol_cfg['tv']:<10} {tf_name:<6} ~{est:>10,} bars")
    ...
    try:
        for tf_name in timeframes:
            ...
            for symbol_cfg in symbols:
                results.append(
                    sync_symbol_tf(fetcher, symbol_cfg, tf_name, date_from, date_to, args.dry_run, resume_state)
                )
    finally:
