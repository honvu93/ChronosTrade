...
import { createTradingReconciliationWorker } from '../queues/tradingReconciliationQueue';
import { TradingReconciliationProcessor } from '../services/trading/TradingReconciliationProcessor';
import { createWorkerLogger } from '../utils/workerLogger';

dotenv.config();

const log = createWorkerLogger('TradingReconciliationWorker');
...
async function main() {
    ...
    const processor = new TradingReconciliationProcessor(prisma);
    const worker = createTradingReconciliationWorker(
        async ({ accountId, requestedByUserId }) => {
            await processor.process({ accountId, requestedByUserId });
        },
        process.env,
    );

    worker.on('completed', (job) => {
        log.info('job_completed', 'Reconciliation completed.', job.id);
    });
